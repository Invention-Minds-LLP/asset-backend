 ...
 * ----------------------
 * Used when onboarding an institute that has 5+ years of pre-existing assets
 * with audited FA register data. The engine NEVER recomputes depreciation for
 * periods before migrationDate; it accepts the audited closing values as-is
 * and resumes calculations from the migration FY onwards.
 *
 * Three modes:
 *   GRANULAR        — full asset-level history known (per-asset originalCost,
 *                     originalPurchaseDate, accDepAtMigration)
 *   PROPORTIONAL    — only category/pool totals known; system pro-rates per asset
 *                     against the asset's current cost share
 *   CARRY_AS_NEW    — no pre-system history derivable; treat as a fresh asset
 *                     purchased on migrationDate with cost = audited NB
 */
import { Response } from "express";
import prisma from "../../prismaClient";
import { AuthenticatedRequest } from "../../middleware/authMiddleware";
import { getFYContext } from "../../utilis/depreciationEngine";

const VALID_MODES = ["GRANULAR", "PROPORTIONAL", "CARRY_AS_NEW"] as const;
type MigrationMode = typeof VALID_MODES[number];

// ── POST /legacy-migration/single — migrate one asset ────────────────────────
export const migrateSingleAsset = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      assetId,
      migrationMode,
      ...
      originalCost,
      accDepAtMigration,
      openingWdvAtMigration,
      auditedBookValueAtMigration,
      auditReferenceId,
      migrationNotes,
    } = req.body;
    ...
    const cost = Number(originalCost ?? asset.purchaseCost ?? 0);
    const accDep = Number(accDepAtMigration ?? 0);
    const openingWdv = Number(openingWdvAtMigration ?? Math.max(0, cost - accDep));
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const u = await tx.asset.update({
        ...
        data: {
          ...
          migrationApprovedById: req.user?.employeeDbId ?? null,
      ...
      if (asset.depreciation) {
        ...
        await tx.assetDepreciation.update({
          ...
          data: {
            ...
            lastCalculatedAt: getFYContext(migDate).fyStart,
          } as any,
        });
      }
...
export const migrateBulk = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!VALID_MODES.includes(migrationMode as MigrationMode)) {
      res.status(400).json({ message: `migrationMode must be one of ${VALID_MODES.join(", ")}` });
      return;
    }

    const migDate = new Date(migrationDate);
    const fyStart = getFYContext(migDate).fyStart;
    ...
    for (const it of items) {
      try {
        ...
        if (!asset) { errors.push({ assetId: it.assetId, error: "Not found" }); continue; }

        const cost = Number(it.originalCost ?? asset.purchaseCost ?? 0);
        const accDep = Number(it.accDepAtMigration ?? 0);
        const openingWdv = Number(it.openingWdvAtMigration ?? Math.max(0, cost - accDep));
...
export const migrateProportional = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (poolId)     where.assetPoolId = Number(poolId);

    const assets = await prisma.asset.findMany({ where, include: { depreciation: true } });
    if (!assets.length) { res.status(404).json({ message: "No assets matched" }); return; }

    const sumOfCosts = assets.reduce((s, a) => s + Number(a.purchaseCost ?? 0), 0);
    if (sumOfCosts <= 0) {
    ...
    await prisma.$transaction(async (tx) => {
      for (const a of assets) {
        const ratio = Number(a.purchaseCost ?? 0) / sumOfCosts;
...
export const listMigratedAssets = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { mode, fromDate, toDate } = req.query;
    const where: any = { isLegacyAsset: true };
    if (mode) where.migrationMode = String(mode);
    if (fromDate || toDate) {
      where.migrationDate = {};
      if (fromDate) where.migrationDate.gte = new Date(String(fromDate));
      if (toDate)   where.migrationDate.lte = new Date(String(toDate));
    }

    const assets = await prisma.asset.findMany({
      where,
      orderBy: { migrationDate: "desc" } as any,
      select: {
    ...
    const enriched = assets.map((a: any) => {
      const variance = a.auditedBookValueAtMigration != null
        ? Number(a.openingWdvAtMigration ?? 0) - Number(a.auditedBookValueAtMigration)
        : null;
      return {
        ...a,
        varianceVsAudited: variance,
        varianceFlagged: variance != null && Math.abs(variance) > 1,
      };
    });
...
export const revertMigration = async (req: AuthenticatedRequest, res: Response) => {
  try {
